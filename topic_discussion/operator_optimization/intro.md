...
![算子优化学习路线：从语义正确到端到端验证](../../docs/public/topic_discussion/operator_optimization/operator_optimization_task_route.svg)

上图先给出 Task0–6 的学习顺序；下表再提供每个任务的具体 Notebook、项目入口和正文索引。

| Task | 学习内容 | 主学习线 / 项目入口 | 学习顺序 | 专题正文 |
|:---|:---|:---|:---|:---|
| Task0 | 算子语义、GPU 执行与性能边界 | [Part 02 · 00 PyTorch Warmup](../../02_PyTorch_Algorithms/00_PyTorch_Warmup.ipynb) → [Part 01 · 08 编程模型](../../01_Hardware_Math_and_Systems/08_Programming_Models_CUDA_Triton.ipynb) → [Part 01 · 15 CUDA 执行模型](../../01_Hardware_Math_and_Systems/15_CUDA_Execution_Model.ipynb) | 数学语义 → 并行执行 → 性能指标 | [01 为什么需要算子优化](./01_why_operator_optimization_matters.md) |
| Task1 | Triton Kernel 基础 | [Part 02 · 01 RMSNorm](../../02_PyTorch_Algorithms/01_RMSNorm_Tutorial.ipynb) → [Part 01 · 18 Triton Block 模型](../../01_Hardware_Math_and_Systems/18_Triton_Block_Model.ipynb) → [Part 03 · 01 Triton 向量加法](../../03_Triton_Kernels/01_Triton_Vector_Addition.ipynb) → [Part 03 · 04 Triton GEMM](../../03_Triton_Kernels/04_Triton_GEMM_Tutorial.ipynb) | tile → load/store → kernel → 正确性 | [02 Kernel 语义与内存访问](./02_kernel_semantics_and_memory.md) |
| Task2 | 访存、布局与片上存储 | [Part 01 · 16 Warp / Block / Shared Memory](../../01_Hardware_Math_and_Systems/16_Warp_Block_SharedMemory_Basics.ipynb) → [Part 01 · 24 SRAM 优化](../../01_Hardware_Math_and_Systems/24_SRAM_Optimization_Techniques.ipynb) → [Part 03 · 12 Triton 内存模型](../../03_Triton_Kernels/12_Triton_Memory_Model_and_Debug.ipynb) | global memory → SRAM → layout → occupancy | [02 Kernel 语义与内存访问](./02_kernel_semantics_and_memory.md) |
| Task3 | 算子融合与模型组件 Kernel | [Part 02 · 02 SwiGLU](../../02_PyTorch_Algorithms/02_SwiGLU_Activation.ipynb) → [Part 02 · 04 Attention](../../02_PyTorch_Algorithms/04_Attention_MHA_GQA.ipynb) → [Part 01 · 19 算子融合基础](../../01_Hardware_Math_and_Systems/19_Operator_Fusion_Introduction.ipynb) → [Part 03 · 03 融合 RMSNorm](../../03_Triton_Kernels/03_Triton_Fused_RMSNorm.ipynb) → [Part 03 · 06 融合 Softmax](../../03_Triton_Kernels/06_Triton_Fused_Softmax.ipynb)；扩展 [Part 02 · 20 FlashAttention 模拟](../../02_PyTorch_Algorithms/20_FlashAttention_Sim.ipynb)、[Part 03 · 08 FlashAttention](../../03_Triton_Kernels/08_Triton_Flash_Attention.ipynb) | 组件结构 → 依赖 → 中间张量 → fusion → 端到端 | [03 融合与 Kernel 组合](./03_fusion_and_kernel_composition.md) |
| Task4 | Tensor Core、CUDA 与异步执行 | [Part 01 · 23 Tensor Core 深入](../../01_Hardware_Math_and_Systems/23_TensorCore_Deep_Dive.ipynb) → [Part 04 · 03 Tensor Core MMA](../../04_CUDA_and_System_Optimization/03_Tensor_Core_MMA_Programming.md) → [Part 04 · 04 Warp Level Primitives](../../04_CUDA_and_System_Optimization/04_Warp_Level_Primitives.md) → [Part 04 · 16 CUDA Shared Memory](../../04_CUDA_and_System_Optimization/16_CUDA_Shared_Memory_Optimization.ipynb) | instruction → warp / block → shared memory → kernel | [04 CUDA 执行与硬件约束](./04_cuda_execution_and_hardware_constraints.md) |
| Task5 | Autotune、Profiling 与 Kernel 选择 | [Part 02 · 44 自动调优框架](../../02_PyTorch_Algorithms/44_Auto_Tuning_Framework.ipynb) → [Part 03 · 05 Triton 自动调优](../../03_Triton_Kernels/05_Triton_Autotune_and_Profiling.ipynb) → [Part 04 · 17 CUDA Stream](../../04_CUDA_and_System_Optimization/17_PyTorch_CUDA_Streams_and_Transfer.ipynb) → [Part 04 · 18 CUDA Graph](../../04_CUDA_and_System_Optimization/18_CUDA_Graph_and_JIT_Compile.md)；扩展 [Part 01 · 13 性能分析与瓶颈定位](../../01_Hardware_Math_and_Systems/13_Profiling_and_Bottleneck_Analysis.ipynb) | 候选参数 → 搜索 → trace → 异步执行 | [05 成本模型与性能分析](./05_cost_model_and_profiling.md) |
| Task6 | Block 与端到端项目验证 | [Part 02 · 05 LLaMA3 Block](../../02_PyTorch_Algorithms/05_LLaMA3_Block_Tutorial.ipynb) → [Part 03 · 13 Triton Llama3 Block 项目](../../03_Triton_Kernels/13_Triton_Llama3_Block_Project.ipynb) → [Part 04 · 21 CUDA / Triton / PyTorch 对照](../../04_CUDA_and_System_Optimization/21_CUDA_vs_Triton_vs_PyTorch.ipynb) → [Part 02 · 74 Profiling 驱动的端到端优化](../../02_PyTorch_Algorithms/74_Profiling_Driven_End_to_End_Optimization.ipynb) | 算子 → Block → 实现路径 → workload → 决策 | [06 基准测试与项目验证](./06_benchmark_and_project_validation.md) |

![算子优化知识地图：语义、数据路径、硬件执行与验证证据](../../docs/public/topic_discussion/operator_optimization/operator_optimization_knowledge_map.svg)
...
|:---|:---|:---|
| [Part 01 · 08 编程模型](../../01_Hardware_Math_and_Systems/08_Programming_Models_CUDA_Triton.ipynb)、[Part 01 · 15 CUDA 执行模型](../../01_Hardware_Math_and_Systems/15_CUDA_Execution_Model.ipynb) | Triton、CUDA、PyTorch 和 GPU 执行边界 | 某个实现一定更快 |
| [Part 01 · 16 Warp / Shared Memory](../../01_Hardware_Math_and_Systems/16_Warp_Block_SharedMemory_Basics.ipynb)、[Part 01 · 23 Tensor Core](../../01_Hardware_Math_and_Systems/23_TensorCore_Deep_Dive.ipynb) | 并行层次、同步和矩阵指令 | 一个 tile 配置适合所有 GPU |
| [Part 01 · 19 算子融合](../../01_Hardware_Math_and_Systems/19_Operator_Fusion_Introduction.ipynb)、[Part 01 · 24 SRAM 优化](../../01_Hardware_Math_and_Systems/24_SRAM_Optimization_Techniques.ipynb) | 中间张量、数据驻留和读写代价 | fusion 一定降低端到端延迟 |
| [Part 01 · 13 性能分析与瓶颈定位](../../01_Hardware_Math_and_Systems/13_Profiling_and_Bottleneck_Analysis.ipynb) | 建立 kernel 和系统热点证据 | 单次 trace 能代表所有 workload |
...
![算子优化证据流：从候选实现到项目决策](../../docs/public/topic_discussion/operator_optimization/operator_optimization_evidence_flow.svg)
