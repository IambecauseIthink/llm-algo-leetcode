  ...
  <main class="mf-shell">
    <section class="mf-hero">
      <div>
        ...
        <h1>进入模型内部，<br><span>把 FP32 巨型仓库压进 INT8。</span></h1>
        <p class="mf-lead">不要先背公式。先跟着一个 token 穿过整座模型工厂，找到真正占显存的 Linear 权重，再亲手拆开、校准、铸造和上线。</p>
      </div>
      <div class="mf-crisis">
        ...
        <div class="mf-crisis-stats">
          <span><small>FP32 权重仓</small><b id="fp32-total">—</b></span>
          <span><small>INT8 改造后</small><b id="int8-total">—</b></span>
          <span><small>压缩率</small><b>4.00×</b></span>
        </div>
        <p id="preset-note">玩具模型会真实计算每个数值；比例与大模型一致。</p>
      </div>
    </section>

    <nav class="mf-mission-rail" aria-label="任务进度">
      <button class="active" data-mission-tab="0"><span>00</span><b>显存危机</b><small>找到权重仓</small></button>
      <button data-mission-tab="1"><span>01</span><b>拆开 Linear</b><small>看到每个字节</small></button>
      ...
      <button data-mission-tab="5"><span>★</span><b>上线挑战</b><small>同时满足四项指标</small></button>
    </nav>
        ...
        <div class="mf-world" id="factory-world" data-view="0" data-lens="flow">
          ...
          <div class="mf-scene byte-scene" data-scene="4">
            <div class="mf-byte-lab">
              <section>
                <p>原始 FP32 权重 <code>2.5</code></p>
                <div class="mf-byte-row fp32-bytes">
                  <span>0100 0000</span><span>0010 0000</span><span>0000 0000</span><span>0000 0000</span>
                </div>
                <small>32 bit = 4 byte</small>
              </section>
              <div class="mf-compressor"><span>× scale</span><b>量化压缩舱</b><span>round · clamp</span></div>
              <section>
                <p>INT8 权重 <code>106</code></p>
                <div class="mf-byte-row int8-bytes"><span>0110 1010</span></div>
                <small>8 bit = 1 byte</small>
              </section>
            </div>
            <div class="mf-byte-verdict"><b>同样数量的权重</b><span>相对 FP32：4× 更小</span><span>相对 FP16：2× 更小</span></div>
          </div>
    ...
    <section class="mf-mission-deck">
      <article class="mf-mission-panel active" data-mission-panel="0">
        <div class="mf-mission-copy">
          ...
          <h2>先别急着量化：模型里到底谁最占地方？</h2>
          <p>下面三台机器都会参与前向，但只有一类拥有巨大的二维权重矩阵。选中真正的主仓库，工厂才会开放下一层。</p>
        </div>
        <div class="mf-choice-grid">
          <button data-hunt="norm"><b>RMSNorm</b><span>每个 hidden channel 一组缩放参数</span></button>
          <button data-hunt="residual"><b>Residual Add</b><span>把主干与支路结果相加</span></button>
          <button data-hunt="linear"><b>Linear Projection</b><span>保存 [out_features, in_features] 权重矩阵</span></button>
        </div>
        <div class="mf-feedback" id="hunt-feedback">提示：参数量不只看“有没有参数”，还要看参数 shape。</div>
      </article>

      <article class="mf-mission-panel" data-mission-panel="1">
        <div class="mf-mission-copy">
          <p class="mf-kicker">MISSION 01 · LINEAR AUTOPSY</p>
          <h2>从整座模型一路拆到 1 个权重</h2>
          <p>沿着左侧五级剖视轨道深入。到达字节层后，亲手把 4 个 FP32 字节装进 1 个 INT8 单元。</p>
        </div>
        <div class="mf-action-row">
          <button class="mf-button hot" id="unfold-bytes" type="button">展开一个 FP32 权重</button>
      ...
      <article class="mf-mission-panel boss-panel" data-mission-panel="5">
        <div class="mf-mission-copy">
          ...
          <h2>让整座模型在预算内上线</h2>
          <p>四条工程约束必须同时满足。通关后，模型里所有可采用 weight-only 量化的 Linear 仓库都会点亮。</p>
        </div>
        <div class="mf-boss-grid">
          ...
          <ul class="mf-boss-checks" id="boss-checks">
            <li data-boss="memory"><i></i><span>权重仓相对 FP32 缩小 4 倍</span><b>4.00×</b></li>
