...
![推理优化从请求症状到部署决策](../../public/topic_discussion/inference_optimization/inference_optimization_story.svg)
...
![KV Cache 生命周期](../../public/topic_discussion/inference_optimization/kv_cache_lifecycle_zh.svg)
...
这一段要回答的不是“哪种调度一定更快”，而是：当前请求分布是否已经需要 Continuous Batching、Chunked Prefill 或 Prefill / Decode 分离。CPU 可以帮助理解队列和批次状态变化；调度收益、P99、跨实例通信和 GPU 利用率仍需真实 backend 验证。
