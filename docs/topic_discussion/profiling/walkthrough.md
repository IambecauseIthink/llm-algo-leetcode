...
它是一条横向支撑路线。训练显存问题回到 `73 → 76 → 75 → 74`，推理问题回到 `66 → 67 → 68 → 69 → 70`，多卡问题回到 `79 → 80 → 81`；Profiling 负责帮助这些项目定位瓶颈和检查证据是否充分。

下面五段分别对应入口中的 Task0–6：前两段建立问题和 baseline，中间两段读取时间、显存与通信证据，最后一段用 benchmark 和回归结果收口。需要快速查字段、工具和判断条件时，回到[性能分析正文](./casebook.md)。
