...
|:---|:---|:---|:---|
| KV Cache 增长 | 历史 token 的运行时驻留 | 每 token、每请求和每并发的容量 | [11 KV Cache](../../01_Hardware_Math_and_Systems/11_KV_Cache_and_Memory_Growth.md) |
| Paging | cache block 的组织和分配 | 碎片、可用容量和回收 | [22 PagedAttention](../../02_PyTorch_Algorithms/22_vLLM_PagedAttention.md) |
| Prefix reuse | 重复前缀的存储复用 | 命中率、复用容量和失效 | [34 Prefix Cache](../../02_PyTorch_Algorithms/34_Prefix_Caching_and_Chunked_Prefill.md) |
...
3. 预算仍不足时，再转入 Task5 判断 KV Cache 量化，或进入架构扩展，例如 [71 MLA / KV Cache](../../02_PyTorch_Algorithms/71_MLA_KV_Cache_Architecture_Benchmark.md)。
...
CPU 可以验证 cache shape、容量公式、分页和前缀匹配逻辑；真实命中率、eviction、并发容量和 backend 显存必须由 [66 推理 baseline](../../02_PyTorch_Algorithms/66_Inference_Performance_Comparison.md) 或 [69 Prefix Cache benchmark](../../02_PyTorch_Algorithms/69_Prefix_Caching_Benchmark.md) 验证。缓存模型不能直接写成 vLLM 或 SGLang 的实测结论。
