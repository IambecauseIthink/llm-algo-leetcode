...
Task0 解释计算图和状态生命周期，Task1 的 [01 数据类型](../../01_Hardware_Math_and_Systems/01_Data_Types_and_Precision.md)、[02 参数量与 FLOPs](../../01_Hardware_Math_and_Systems/02_LLM_Params_and_FLOPs.md)、[03 GPU 架构与显存](../../01_Hardware_Math_and_Systems/03_GPU_Architecture_and_Memory.md) 和 [06 显存计算与 ZeRO](../../01_Hardware_Math_and_Systems/06_VRAM_Calculation_and_ZeRO.md) 再把对象放入硬件和状态账本。需要连接 Attention 访存时，补看 [20 FlashAttention Sim](../../02_PyTorch_Algorithms/20_FlashAttention_Sim.md)。

后续验证出口按问题选择：训练侧进入 [73 训练性能分析](../../02_PyTorch_Algorithms/73_Training_Performance_Analysis.md)、[76 策略对比](../../02_PyTorch_Algorithms/76_Activation_Checkpoint_Offload_Benchmark.md) 和 [74 Profiling 收口](../../02_PyTorch_Algorithms/74_Profiling_Driven_End_to_End_Optimization.md)；推理侧进入 [04 推理 Cache 与显存预算](./04_inference_cache_and_memory_budget.md)。
