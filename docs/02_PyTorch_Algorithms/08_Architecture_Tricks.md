...
本节选两个典型 architecture tricks 做最小实现：权重绑定，以及 Gemma 风格的 `1 + w` RMSNorm 缩放。完成后，你应该能看懂这些“看起来很小”的结构差异为什么值得单独讨论，并能在阅读不同模型代码时快速定位它们改变了哪条参数或梯度路径。

本节还提供一张 Attention 结构变体地图：MHA / GQA / MQA 改变 KV 头的共享方式，Sparse Attention 改变 token 连接模式，Linear Attention 改变计算顺序和状态表示，FlashAttention 则主要优化 dense Attention 的执行与访存。你会用小规模 CPU 观察这些机制的输入、状态和输出差异，再把真实吞吐问题交给后续 GPU 实验。
...
**导语：** 先能在 RMSNorm、Decoder Layer 和 `nn.Module` 中定位参数与计算路径，再把结构变体对应到具体代码位置。
...
![架构变体：从结构改动追踪系统影响](../public/02_PyTorch_Algorithms/08_architecture_tricks.svg)
...
**不要把模型名称当成结构结论：** 是否绑定应以模型 config 和实际模块指针为准；同一模型家族的不同版本也可能采用不同配置。`data_ptr()` 能证明当前 Python 模块是否共享存储，但不能证明所有 checkpoint 或部署 backend 都保持相同布局。

### Step 2.5: Attention 结构变体地图

| 变体 | 改变什么 | CPU-first 可以观察什么 | 不能在本节直接断言什么 |
|:---|:---|:---|:---|
| MHA | 每个 Q head 对应独立 K/V head | `num_attention_heads == num_key_value_heads` | 实际 decode 一定更快 |
| GQA / MQA | 多个 Q head 共享 K/V head | KV 投影规模与 head 分组关系 | KV Cache 一定按理论比例下降 |
| Sparse Attention | 只保留部分 token 连接 | mask 非零率和连接数量 | 稀疏 mask 一定带来 kernel 加速 |
| Linear Attention | 改写注意力计算顺序并维护状态 | toy recurrence 的 shape 和状态更新 | 所有任务都能保持 dense attention 的质量 |
| FlashAttention | 融合 dense attention 的执行和访存 | 与 dense 公式的数值对应关系 | 它本身是新的 attention 连接模式 |
...
再用 CPU 小张量完成三项观察：用 `data_ptr()` 验证共享存储，用参数量公式比较 tied / untied，用一个 toy mask 统计 Sparse Attention 的连接比例。最后说明这些观察为什么仍然不能替代 GPU kernel benchmark。
...
def summarize_attention_variant(num_attention_heads, num_key_value_heads, seq_len, window_size=None):
    """TODO 3: 统计 MHA/GQA/MQA 类型和 Attention 连接规模。"""
    # 提示：先判断 Q/KV head 关系；window_size=None 表示 dense，否则统计局部窗口连接数。
    # attention_type = ???
    # total_connections = ???
    # connection_density = ???
...
def test_tricks():
    try:
        hidden_size = 64
        vocab_size = 1000
        variant = summarize_attention_variant(8, 2, seq_len=8, window_size=1)
        assert variant['attention_type'] == 'GQA'
        assert variant['total_connections'] == 22
        assert 0 < variant['connection_density'] < 1
...
def summarize_attention_variant(num_attention_heads, num_key_value_heads, seq_len, window_size=None):
    """TODO 3: 统计 MHA/GQA/MQA 类型和 Attention 连接规模。"""
    if num_attention_heads <= 0 or num_key_value_heads <= 0 or seq_len <= 0:
        raise ValueError("head 数和 seq_len 必须为正数")
    if num_attention_heads % num_key_value_heads != 0:
        raise ValueError("num_attention_heads 必须能被 num_key_value_heads 整除")
    attention_type = 'MHA' if num_attention_heads == num_key_value_heads else ('MQA' if num_key_value_heads == 1 else 'GQA')
    if window_size is None:
        total_connections = seq_len ** 2
    else:
        if window_size < 0:
            raise ValueError("window_size 不能为负数")
        total_connections = sum(min(seq_len, i + window_size + 1) - max(0, i - window_size) for i in range(seq_len))
    return {
        'attention_type': attention_type,
        'total_connections': total_connections,
        'connection_density': total_connections / (seq_len ** 2),
    }
...
**3. TODO 3：统计 Attention 结构与连接规模**
- **实现方式**：先根据 Q head / KV head 的整除关系识别 MHA、GQA 或 MQA；`window_size=None` 时按 dense 连接统计，否则按局部窗口统计。
- **关键点**：连接密度是结构账本，不是 kernel 性能；Sparse Attention 是否更快要交给真实实现和 GPU benchmark 验证。
...
结构变体适合回到真实模型配置和开源实现中核对；当结构改动影响执行路径时，再进入精度、CUDA/Triton 和图优化专题。

- [Transformers 模型实现仓库](https://github.com/huggingface/transformers)
- [Qwen3 开源仓库](https://github.com/QwenLM/Qwen3)
- [Gemma 开源仓库](https://github.com/google-deepmind/gemma)
- [Grouped-Query Attention 原论文](https://arxiv.org/abs/2305.13245)
- [10. LoRA 激活微调](../02_PyTorch_Algorithms/10_LoRA_Tutorial.md)
- [P1: Tensor Core 与混合精度](../01_Hardware_Math_and_Systems/12_TensorCore_and_Mixed_Precision.md)
- [P1: 编程模型：CUDA 与 Triton](../01_Hardware_Math_and_Systems/08_Programming_Models_CUDA_Triton.md)
- [P1: AI 编译器与计算图优化](../01_Hardware_Math_and_Systems/09_AI_Compilers_and_Graph_Optimization.md)
