...
本节要求你根据真实请求的复用模式，判断前缀缓存是否值得部署。先固定请求分布、chunk 策略和 baseline，再比较 hit rate、TTFT、prefill 节省量与缓存维护开销。默认用一个稳定 backend 完成 G0 / G1；SGLang 的 RadixAttention 作为可选扩展，不与基础对照混成同一个变量。
...
**导语：** 完成前缀缓存 benchmark 后，可用 70 继续检查缓存命中与请求调度之间的关系，并保持相同的 workload 记录方式。
...
### Step 1: 明确 benchmark 的输入、变量和输出

本项目的输入是一组已经 token 化的请求样本。先固定共享前缀、后缀分布、生成长度、并发和 backend，再只改变 cache policy，才能把收益归因到前缀复用。

| 项目 | 本项目中的约定 |
|---|---|
| 输入 | shared-prefix 请求集、生成长度、并发度、模型和 backend |
| 单变量 | cache off / prefix cache；扩展时再比较 chunk 粒度或 backend |
| 观察指标 | hit rate、reused tokens、TTFT、TPOT、吞吐、P99、峰值显存和维护开销 |
| 输出 | baseline/candidate 对照表，以及 `accept / tune / reject` 决策 |

### Step 2: 先验证请求分布和 baseline

共享前缀必须在 token 化后保持一致。先运行 cache off 的 G0，再开启一种 cache policy 运行 G1：
...
| 命中证据 | 不适用 | backend metrics、日志或专用接口 |

### Step 3: 用统一口径比较收益与代价

命中率不是最终目标。每组结果至少要同时记录：

| 指标组 | 字段 | 判断问题 |
|---|---|---|
| 复用 | hit rate、reused tokens、prefill work reduction | 重复计算减少了多少 |
...
### Step 4: 输出 benchmark 决策

| 决策 | 条件 | 下一步 |
|---|---|---|
| accept | 命中率达标，TTFT/吞吐有稳定收益，维护代价可接受 | 扩大请求量和并发回归 |
| tune | 命中率达标但收益不稳定，或 chunk/失效策略仍需调整 | 改变单一变量后重跑 |
| reject | 命中率不足、代价过高、OOM 或无端到端收益 | 关闭缓存或更换 workload |
...
**统一结果表：** 先填写共同条件，再填写每个实验组的结果；同一行内的模型、backend、workload 和生成长度必须保持可追溯。

| 实验组 | model / backend / dtype | prompt / generated tokens | batch / concurrency | cache policy | hit rate / evidence | reused tokens | TTFT (ms) | TPOT (ms/token) | E2E (ms) | P99 (ms) | throughput (token/s) | peak memory (MiB) | quality / status | decision |
|---|---|---:|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|
| G0 | 待填写 | 待填写 | 1 / 1 | off | 不适用 | 0 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待填写 | 待判断 |
| G1 | 待填写 | 待填写 | 1 / 1 | prefix cache | 待采集 / metrics 或日志 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待填写 | 待判断 |
| G2 | 待填写 | 待填写 | 待填写 | 待填写 | 待采集 / 证据来源 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待采集 | 待填写 | 待判断 |
| 共同条件 | 模型 revision、服务版本、GPU、PyTorch/CUDA、workload 文件和 warmup/重复次数 | 与 G0/G1 保持一致 | 变量只在说明中列出 | 记录实际开关 | 命中率不能由 TTFT 推断 | 记录 token 口径 | 单位统一 | 单位统一 | 单位统一 | P99 口径统一 | 输出 token/s | `torch.cuda.max_memory_allocated` 或 backend 指标 | 记录 OOM 与质量 | accept / tune / reject |
...
按 66 的统一分组执行：G0 关闭 prefix cache，G1 在相同 shared-prefix workload 下开启一种 cache policy，G2 才比较不同 backend 或缓存策略。建议先用 vLLM 或当前环境中唯一稳定的 backend 完成 G0 / G1；SGLang 作为 G2 扩展。普通服务启动成功不代表 prefix cache 已启用。
