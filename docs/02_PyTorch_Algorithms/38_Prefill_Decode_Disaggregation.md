...
一次请求先处理输入，再逐 token 生成输出。先按请求记录这两段的规模，才能判断拆分是否有依据。
...
先把每个请求的输入长度、生成长度和当前状态记录下来。这样可以从同一份账本识别流量构成，再比较共享池和拆分方案的资源需求。
...
一条请求账本可以写成“请求 a：输入 4000 token、生成 64 token、命中前缀、位于 prefill 队列”。前 3 个核心字段用于判断请求形态，后 2 个状态字段用于观察复用和排队变化；它们共同构成后续拆分判断的输入。

### Step 3: 比较拆分收益与交接代价

PD 分离的核心不是“拆开就更快”，而是判断流量分化后，独立资源池带来的收益能否覆盖请求交接和资源组织成本。先从吞吐、延迟和资源使用观察收益，再把交接时间、缓存迁移和空闲容量纳入判断。
...
CPU 机制实验可以比较吞吐和延迟方向；交接时间、KV Cache 迁移和 worker 空闲容量需要在真实 backend 中测量。

![PD 分离的流量分类、资源交接与指标决策机制](../public/02_PyTorch_Algorithms/38_pd_decision_mechanism.svg)
...
完成 CPU 测试后，再把同样的请求账本和指标字段带入 Step 5；Step 5 只负责说明真实 backend / 多 GPU 证据需要哪些条件。
...
真实 backend 验证优先参考 [70 Serving Scheduler Benchmark](./70_Serving_Scheduler_Benchmark.md) 和 [81 Distributed Inference Project](./81_Distributed_Inference_Project.md)。本节 GPU 结果必须记录 `evidence_level`，CPU 模拟结果不能写成真实 PD 分离收益。
