...
**导语：** 进入本项目后，先用 LoRA 机制、有效 batch 和端到端训练闭环固定实验口径，再比较资源成本、质量变化和 adapter 交付条件。
...
本项目的输入是模型、指令数据、LoRA 配置和训练条件；操作是分别运行全参数更新与 LoRA 更新；输出是参数量、训练速度、显存、loss 和 adapter 报告。成功条件是：两组对照口径一致，LoRA 的资源成本有明确变化，验证效果处于可接受范围，并且产物可以被复核。
...
| 对照变量 | 用示例数字检查 baseline / LoRA 的比较逻辑 | baseline 更新底座权重；LoRA 冻结底座，只更新 LoRA 适配器 | 只改变参数更新方式；模型、数据和训练条件保持一致 |
| 固定训练条件 | 不运行模型训练，只检查配置字段和对照口径 | 两组使用相同的模型、数据、切分、tokenizer、dtype、micro-batch、序列长度、优化器、学习率和步数 | 具体参数在 Step 2 固定 |
| 输出与判断 | 生成数据审计、参数账本、指标汇总和决策逻辑；不包含真实训练指标 | 生成 train / val loss、可训练参数、峰值显存、step time，并保存 LoRA adapter 与 JSON 报告 | 根据资源变化、验证效果和产物完整性判断是否进入下一步 |
...
Step 6 是 GPU 实验入口，先回答一个问题：当前环境能否加载选定的真实模型和数据，并完成一次最小 LoRA 更新。它的输入是当前 GPU、模型和最小样例，输出是运行状态与 adapter / tokenizer / JSON 路径；它只检查运行链路与产物，不比较 baseline 和 LoRA。对照实验放在 Step 7。
...
Step 7 做 GPU 对照实验：输入是同一模型、同一批训练 / 验证样本和同一训练条件，分别运行 baseline 与 LoRA；唯一改变训练方式，输出两组可比较的指标和交付记录。`SPLIT_SEED` 固定数据切分，`REAL_SEED` 用于重复运行。Step 6 的 smoke 结果不能代替 Step 7 的对照结果。
...
完成 LoRA 的资源、质量和 adapter 交付比较后，可以继续阅读参数高效微调实现与训练性能项目。
